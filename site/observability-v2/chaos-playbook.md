...
!!! warning "Shop has no chaos write endpoints"
    ...
    (`https://crm.<DNS_DOMAIN>/admin/chaos`) or from the Ops / cp
...
```bash
curl -sS https://crm.<DNS_DOMAIN>/api/admin/chaos/apply \
...
```bash
curl -sS -X POST https://crm.<DNS_DOMAIN>/api/admin/chaos/clear -b session.cookie
```
