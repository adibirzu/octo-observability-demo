...
Playwright specs that run against a **deployed tenancy**, not the local
dev stack. They are opt-in so CI and local verification can stay fast
...
| `tests/e2e/full-platform-smoke.spec.ts` | `FULL_PLATFORM_E2E_ENABLED=1` | Shop/CRM readiness plus optional `load-control`, `remediator`, and `object-pipeline` reachability |
...
If DNS is not propagated yet but ingress is healthy, point both base URLs at the ingress IP and pass the expected virtual host names:
