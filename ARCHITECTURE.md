...
**APM enhanced from Langfuse.** `services/genai-studio/app/sync/langfuse_apm_sync.py` periodically pulls the Langfuse public API (tokens, cost, latency p50/p95, LLM-as-judge scores) and publishes them as OCI Monitoring custom metrics (namespace `octo_genai`), so the token/cost dashboard (`deploy/oci/monitoring-dashboards/genai-token-cost.json`) and alarms read them in OCI. On OKE this runs as the hourly `octo-genai-langfuse-apm-sync` CronJob in `octo-drone-shop`; operators can create a one-off backfill job from that CronJob after rollout instead of waiting for the next schedule. APM saved queries (`deploy/oci/apm/saved-queries/ai-studio-*.json`) carry **external drilldowns** to `langfuse.${DNS_DOMAIN}` and `grafana.${DNS_DOMAIN}`. LLM-as-a-judge runs as Langfuse evaluator templates scored by an OCI GenAI model (`services/genai-studio/scripts/ensure_evaluator_templates.py`).

**External observability stack.** Langfuse + Grafana are deployed by the optional `services/observability-stack/` component into their own namespaces (`octo-langfuse`, `octo-grafana`) — never part of the app runtime. The studio reads its dedicated AI APM datakey from `octo-apm-ai`, Langfuse project keys from `octo-llmetry`, and OCI Monitoring coordinates from `octo-oci-config`. All secrets are generated at deploy time / read from OCI; nothing sensitive is committed.
