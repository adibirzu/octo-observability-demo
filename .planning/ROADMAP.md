...
- **v1.5 — rum-synthetic-monitor (Phase 11 candidate):** Extend `octo-traffic-generator` to launch Playwright + Chromium for a configurable % of sessions (default ~10%). Real browser execution fires the embedded `apmrum.min.js` agent, populating OCI APM RUM dashboards (Page Load timing, Session, Geo, Browser, Custom dimensions like `workflow.id`). Current `httpx`-only generator never executes JS → RUM widgets stay empty. Alternative scoping: integrate OCI APM Synthetic Monitoring scripted browser monitors instead of in-cluster Playwright (managed vs self-hosted tradeoff).
